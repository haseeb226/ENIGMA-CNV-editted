...
winpty docker run -it -d --name enigma-cnv-freesurfer \
...
winpty docker exec -it -w //usr/local/freesurfer enigma-cnv-freesurfer yum install unzip

winpty docker exec -it -w //usr/local/freesurfer enigma-cnv-freesurfer fs_install_mcr R2014b

winpty docker exec -it -w //usr/local/freesurfer/myfiles enigma-cnv-freesurfer sh s0_optional_segmentation.sh //usr/local/freesurfer/myfiles --BrainStem --HippocampusAmygdala --ThalamicNuclei --Hypothalamus

winpty docker exec -it -w //usr/local/freesurfer/myfiles enigma-cnv-freesurfer sh s0a_optional_seg_table.sh //usr/local/freesurfer/myfiles --CreateTable
