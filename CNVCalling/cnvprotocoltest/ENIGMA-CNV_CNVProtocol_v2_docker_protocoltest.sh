...
# General info
declare Dataset="enigmatest" # USER-INPUT - Replace test with the name of your dataset
...
declare ANALYSISDIR="$PWD" # USER-INPUT - replace with the full path for the Analysis-folder on your computer/server
...
declare Chip="Illumina_GSA" # USER-INPUT - genotyping chip: Note - if your dataset is Affymetrix, make sure to put in "Affy6" - otherwise the script will not run appropriately
declare Chipversion="v2" # USER-INPUT - version of chip (if applicable) [set as NA if non-applicable]
...
declare IlluminaReport="enigmacnvtest_FinalReport.txt" # USER-INPUT - name of your Illumina-Final Report-file
declare SNPPosFile="enigmacnvtest_SNP_Map_hg19.txt" # USER-INPUT
