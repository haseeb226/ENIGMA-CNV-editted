...
# Needed to override original PlotCNVsFromDataFrame to deal with similar filesnames, i.e 10 & 110
PlotCNVsFromDataFrame <- function(DF, PathRawData=".", Cores=1, Skip=0, LCR=NULL, PlotPosition=10, Pattern="*",recursive=TRUE, dpi=300, Files=NA, SNPList=NULL, key=NA, OutFolder=".", XAxisDefine = NULL, Window=NA, hg="hg19") #
{
  ...
  mclapply(DF$UniqueID, mc.cores=Cores, function(UID)
  {
    ...
    #RawFile <- Files[grep(ID, Files)] #Next line does not work for me. I prefer this line instead. # MODIFIED FROM HERE
    RawFile <- Files[grep(paste("\\b", ID, Pattern, "$", sep = ""), Files)] # MODIFIED TO THIS - this should deal with similar filesnames, i.e 10 & 110
...
assignInNamespace("PlotCNVsFromDataFrame", PlotCNVsFromDataFrame, ns="iPsychCNV")
