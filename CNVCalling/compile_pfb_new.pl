...
my $used_files = 0;
my (%bafsum, %bafcount, %snpchr, %snppos);
for my $i (0 .. @inputfile-1) {
	...
	if ($@) {
		if ($@ =~ m/Too many open files/) {
			print STDERR "NOTICE: File handle cannot be created by the operating system after reading ${\($i+1)} files\n";
			last;
	...
	print STDERR "Reading file $used_files: $inputfile[$i]\n";
	
	my ($header_seg, $baf_index, $chr_index, $pos_index);
	$_ = readline ($fh);
	s/[\r\n]+$//;
	...
	while (<$fh>) {
		...
		if (not defined $snpposfile) {
			$snpchr{$snp} = $field[$chr_index];
			$snppos{$snp} = $field[$pos_index];
		}
		...
		if (not defined $bafcount{$snp}) {
			$bafsum{$snp} = 0;	
			$bafcount{$snp} = 0;	
		}
		$bafsum{$snp} += $field[$baf_index];
		$bafcount{$snp} += 1;
...
print STDERR "NOTICE: A total of ${\($used_files)} input files were used for compiling PFB values\n";
...
foreach my $snp (sort keys %bafcount) {
	my $mean;
	if ($snp =~ m/^cnv/) {
		$mean = 2;
	} elsif ($bafcount{$snp} == 0) {
		$mean = 2;
	} else {
		#print "$bafsum{$snp} / $bafcount{$snp}\t";
		$mean = $bafsum{$snp} / $bafcount{$snp};
